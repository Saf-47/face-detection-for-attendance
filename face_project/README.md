...
> [!TIP]
> For detailed architecture, configuration, and logic explanation, see [PROJECT_DOCUMENTATION.md](PROJECT_DOCUMENTATION.md).
> For methodology, see [METHODOLOGY.md](METHODOLOGY.md).
> For results and limitations, see [RESULTS_AND_LIMITATIONS.md](RESULTS_AND_LIMITATIONS.md).
> For diagrams and charts, see [GRAPHICAL_REPRESENTATIONS.md](GRAPHICAL_REPRESENTATIONS.md).
...
3.  **Prepare Videos**:
    Place your short training videos (approx 10s) in the `videos/` folder.
...
```powershell
python scripts/infer_and_log.py
```
- Press `q` to quit.
...
You can modify `scripts/infer_and_log.py` to change settings:
